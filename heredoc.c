...
/*   Updated: 2024/05/08 15:42:19 by pmagalha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool    get_delimiter(t_lexer *redir, char **delimiter)
{
    ...
    while (head->type != HEREDOC)
        head = head->next;
    if ((head->content[0] == '\"' && head->content[ft_strlen(head->content) -1] == '\"')
		|| (head->content[0] == '\'' && head->content[ft_strlen(head->content) - 1] == '\''))
        quotes = true;
    if (head->content && quotes)
        *delimiter = ft_strndup(head->content + 1, ft_strlen(head->content) - 2);
	else if (head->content && !quotes)
		*delimiter = ft_strndup(head->content, ft_strlen(head->content));
    return (quotes);
}
